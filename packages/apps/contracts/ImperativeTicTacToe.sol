...
contract ImperativeTicTacToe is StateChannel, TwoPlayerGame {
    ...
    correctTurn(state)
  {
    ...
    progressState(abi.encode(state));
  }
    ...
    verifyUnsettledState(abi.encode(state))
  {
    ...
    setResolution(computeResolution(state));

    // StateChannel function to set final state
    progressState(abi.encode(state));
  }

  function getResolution(State state)
    verifySettledState(abi.encode(state))
    external
    returns (Transfer.Details)
  {
    return winnerGetsAll(state.game, terms);
  }

...
- Makes a delegatecall to `executeEffectOfInterprettedAppOutcome` with a limit of `c_1 + c_2` as also included in the terms.
...
> NOTE: Although not shown in the visualization, the order of transactions is important. The `multiSend` must encode the call to `proxyCall` **before** the call to `executeEffectOfInterprettedAppOutcome`.

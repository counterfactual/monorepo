...
The interpreter used and the params to the interpreter are fixed per app instance by being included in the appIdentityHash computation. After an outcome is stored, the adjudication layer allows a commitment to `StateChannelTransaction.sol:executeEffectOfInterprettedAppOutcome` call the intepreter on the outcome.

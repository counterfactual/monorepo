...
export class StateChannel {
  constructor(
    ...
    readonly ethVirtualAppAgreementInstances: ReadonlyMap<
      string,
      ETHVirtualAppAgreement
    > = new Map<string, ETHVirtualAppAgreement>([]),
  ...
  public installETHVirtualAppAgreementInstance(
    evaaInstance: ETHVirtualAppAgreement,
    ...
    const evaaInstances = new Map<string, ETHVirtualAppAgreement>(
    ...
    return new StateChannel(
      ...
      new Map(
        [...Object.values(json.ETHVirtualAppAgreementInstances || [])].map(
          (appInstanceEntry): [string, ETHVirtualAppAgreement] => {
            return [
              appInstanceEntry[0],
              ETHVirtualAppAgreement.fromJson(appInstanceEntry[1])
            ];
          }
        )
      ),

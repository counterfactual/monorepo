...
const getUpdateHash = (
  ...
  ethers.utils.solidityKeccak256(
    ...
    ["0x19", [A.address, B.address], nonce, TIMEOUT, encodedAppState]
  );

const getFinalizeHash = (appState: string, nonce: number) =>
  getUpdateHash(appState, nonce, 0);

StateChannel.link("Signatures", Signatures.address);
StateChannel.link("StaticCall", StaticCall.address);
StateChannel.link("Disputable", Disputable.address);
  ...
  before(async () => {
    ...
    sendSignedUpdateToChainWithNonce = (nonce: number, appState?: string) =>
      stateChannel.functions.setState(
        ...
        Utils.signMessageBytes(
          getUpdateHash(appState || Utils.zeroBytes32, nonce, TIMEOUT),
          [unlockedAccount]
        ),
        Utils.highGasLimit
      );
    ...
    sendSignedFinalizationToChain = async () =>
      stateChannel.functions.setState(
        ...
        0,
        Utils.signMessageBytes(
          getFinalizeHash(await latestState(), await latestNonce()),
          [unlockedAccount]
        ),
        Utils.highGasLimit
      );

...
this.window.cfWallet = (function (exports, utils$1) {
	'use strict';
	...
	class AppInstance {
	    constructor(info, provider) {
	        ...
	        if ("appInterface" in info) {
	            const { appInterface, defaultTimeout } = info;
	            this.appDefinition = appInterface.addr;
	            this.abiEncodings = {
	                stateEncoding: appInterface.stateEncoding,
	                actionEncoding: appInterface.actionEncoding
	            };
	            this.timeout = utils$1.bigNumberify(defaultTimeout);
	        }
	        else {
	            const { appDefinition, abiEncodings, timeout } = info;
	...
	(function (EventType) {
	    ...
	    EventType["CREATE_CHANNEL"] = "createChannel";
	...
	const jsonRpcMethodNames = {
	    [Node.MethodName.GET_STATE_DEPOSIT_HOLDER_ADDRESS]: "chan_getStateDepositHolderAddress",
	...
	class Provider {
	    ...
	    async createChannel(owners) {
	        const response = await this.callRawNodeMethod(Node.RpcMethodName.CREATE_CHANNEL, {
	            owners
	        });
	        const { transactionHash } = response.result;
	        return transactionHash;
	    }
	    async deposit(multisigAddress, amount, notifyCounterparty = true) {
	    ...
	    async withdraw(multisigAddress, amount, recipient) {
	    ...
	    async getFreeBalanceState(multisigAddress) {
	        const response = await this.callRawNodeMethod(Node.RpcMethodName.GET_FREE_BALANCE_STATE, {
	            multisigAddress
	        });
	        const freeBalances = Object.keys(response.result).reduce((freeBalances, key) => {
	            freeBalances[key] = utils$1.bigNumberify(response.result[key]);
	            return freeBalances;
	        }, {});
	        return freeBalances;
	    }

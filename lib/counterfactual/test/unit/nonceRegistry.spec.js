...
contract("NonceRegistry", accounts => {
    let registry;
    const computeKey = (salt) => ethers.utils.solidityKeccak256(["address", "bytes32"], [accounts[0], salt]);
    beforeEach(async () => {
    ...
    it("can set nonces", async () => {
        ...
        ret.finalizesAt.should.be.bignumber.eq((await provider.getBlockNumber()) + 10);
    });
    it("fails if nonce increment is not positive", async () => {
        await Utils.assertRejects(registry.functions.setNonce(Utils.ZERO_BYTES32, 0));
    });
    it("can finalize nonces", async () => {
        ...
        const isFinal = await registry.functions.isFinalized(computeKey(Utils.ZERO_BYTES32), 0);
        isFinal.should.be.equal(true);
    });
});
//# sourceMappingURL=nonceRegistry.spec.js.map
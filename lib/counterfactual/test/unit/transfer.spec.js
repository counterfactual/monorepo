...
contract("Transfer", (accounts) => {
    ...
    describe("Executes delegated transfers for ETH", () => {
        ...
        it("for 1 address", async () => {
            ...
            await delegateProxy.functions.delegate(transfer.address, transfer.interface.functions.transfer.encode([details]), Utils.HIGH_GAS_LIMIT);
        ...
        it("for many addresses", async () => {
            const randomTargets = Array.from({ length: 10 }, () => Utils.randomETHAddress());
            const details = {
